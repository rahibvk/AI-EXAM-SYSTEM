...
export default function TeacherDashboard() {
    const [stats, setStats] = useState({ courses: 0, exams: 0, students: 0 })

    useEffect(() => {
        // Fetch real stats in future
        // For now mocking or fetching simplified lists
        const fetchData = async () => {
            try {
                const courses = await api.get("/courses/my-courses")
                setStats(prev => ({ ...prev, courses: courses.data.length }))
            } catch (e) { }
        }
        fetchData()
    }, [])
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {[
                    ...
                    { label: "Pending Reviews", value: "12", icon: FileCheck, color: "bg-orange-500" },
                    { label: "Total Students", value: "145", icon: Users, color: "bg-emerald-500" }
                ].map((stat, i) => (
                    <motion.div
                        key={i}
